...
bugBuild.prototype.bugCollision = function(index,hitBox = false){ 
  //Hitboxes by default are made from the user
...
bugBuild.prototype.collision = function(collisionIndex){
  if(battleMap[Number(collisionIndex[4][0])][Number(collisionIndex[4][1])] == 2&&collision(collisionIndex[0],collisionIndex[1],collisionIndex[2],collisionIndex[3],this.X-this.Image.width/144*(this.Age/150+0.5),this.Y-this.Image.height/36*(this.Age/150+0.5)-this.Image.height/72,this.Image.width/72*(this.Age/150+0.5),this.Image.height/36*(this.Age/150+0.5))){  
   ...
   this.SwimmingDistance = collisionIndex[3] == 25 && this.Y < (collisionIndex[1]+30) ? Math.abs(collisionIndex[1]-this.Y)+25 : 40;
      if(Math.abs(this.SwimmingDistance-25) <= 10&&!this.Attacking){ soundeffect("Splash.mp3"); }
  }
  ...
  if(this.Y - collisionIndex[1] <= 30&&this.Y - collisionIndex[1] >= 15&&collisionIndex[4][0] > 0&&collisionIndex[4][0] > 0&&battleMap[Number(collisionIndex[4][0])-1][Number(collisionIndex[4][1])] !== 1&&collision(collisionIndex[0]+(collisionIndex[2]/2)-(55/4)-5,collisionIndex[1],collisionIndex[2]-55/2+10,20,this.X-this.Image.width/144*(this.Age/150+0.5),this.Y-this.Image.height/36*(this.Age/150+0.5)-this.Image.height/72,this.Image.width/72*(this.Age/150+0.5),this.Image.height/36*(this.Age/150+0.5))){
   this.Y = collisionIndex[1] + 15;
   this.Hold = true;

   //Touching the floor disables flying
   if(this.Jump == 5&&bugStats[this.Species].flyAble){ this.Jump = 0;soundeffect("FlyEnd.mp3"); } 
   ...
   if(battleBugs.length > 0&&collisionIndex[4][0] > 1&&battleMap[Number(collisionIndex[4][0])-2][Number(collisionIndex[4][1])] !== 1&&bugStats[this.Species].flyAble&&this.Enemy&&this.Y > battleBugs[0].Y&&!(battleBugs.length > 0&&bugStats[this.Species].skillName == "Specialized Ambush"&&Math.abs(this.X-battleBugs[0].X) > 120)){
     if(this.Jump == 0&&this.Hold == true){ soundeffect("FlyStart.mp3");  }
     this.Jump = 10;
   }   
  ...
  if((this.Y-this.Image.height/36*(this.Age/150+0.5)-this.Image.height/72) > collisionIndex[1]){
   ...
   this.Jump = 5;    
    ...
    if(collisionIndex[4][1]-1 > 0&&battleMap[Number(collisionIndex[4][0])][Number(collisionIndex[4][1]-1)] !== 1&&collision(collisionIndex[0],collisionIndex[1],1,collisionIndex[3],this.X-this.Image.width/144*(this.Age/150+0.5),this.Y-this.Image.height/36*(this.Age/150+0.5)-this.Image.height/72,this.Image.width/72*(this.Age/150+0.5),this.Image.height/36*(this.Age/150+0.5))){
       ...
       if(this.Enemy&&(battleBugs.length == 0||(bugStats[battleBugs[0].Species].skillName == "Specialized Ambush"&&battleBugs[0].Fury >= 0.98))) { this.Scale *= -1 }
...
bugBuild.prototype.constants = function(multFactor){
  if(!bugStats[this.Species].obtained){ bugStats[this.Species].obtained = true }
     
  if(bugStats[this.Species].evolve&&this.Age >= 50){ 
      this.defects.evolution = 1;
      if(this.Name.split(" ").join("_") == this.Species){ this.Name = bugStats[this.Species].evolve.split("_").join(" ") }
 ...
 if(!this.Patrol||(this.Patrol.Type !== 8&&this.Patrol.Type !== 9)){ this.Jump = 0 }    
    
 if(this.Patrol&&this.X < 500+this.Patrol.Index*528&&this.X > this.Patrol.Index*528+28){
       
  switch (this.Patrol.Type){ 
      case 0:
      if(this.Health !== this.HealthTotal){ this.Patrol = false; battleBugs.splice(battleBugs.indexOf(this),1); save(); }
      break;
        ...
        **/
        ...
        for(mateSearch in mates){     
            ...
            mates[mateSearch].X += (this.Scale*10);;
                    ...
                    if(bugs[bugs.length-1].Gender == "Female"){ bugs[bugs.length-1].Name = String(this.Name+"ette").substr(0,20) }
      ...
      case 8:
      if(bugStats[this.Species].swimAble){ this.Health = Math.min(this.HealthTotal,this.Health+(5/1800*multFactor)); } 
          else { this.Patrol = false; }
      this.Swimming = true;
      this.SwimmingDistance = 0;
      ...
      this.Jump = 6;
       ...
       this.Jump = 5;
       if((this.Scale == -1&&this.X < (70+this.Patrol.Index*528))||(this.Scale == 1&&this.X > (458+this.Patrol.Index*528))){ this.Scale *= -1 }
      } else {
       if(Math.floor(Math.random()*50) == 0){ this.Jump = this.Jump == 6 ? 5 : 6  }
      }
          
      if(this.Jump > 5){ this.Jump = 6 } else if(this.Jump > 0) { this.Jump = 5 }
      break;
          
      case 9:
      if(bugStats[this.Species].flyAble){ this.Health = Math.min(this.HealthTotal,this.Health+(5/1800*multFactor)); } 
          else { this.Patrol = false; }
  ...
  if(food <= 0&&this.Trait !== 2&&multFactor <= 1){ this.Health = Math.max(0,this.Health-(5/1800*multFactor)); } 
}

bugBuild.prototype.skill = function(victim){ 
   if(this.Fury > 0.5&&bugStats[this.Species].skillName !== "Life Meal"){  this.Attack *= 2; }
   ...
   if(this.Fury > 0.5){  
       this.Fury = 0;
       if(bugStats[this.Species].skillName !== "Life Meal"){ this.Attack /= 2; }
       soundeffect((bugStats[this.Species].skillName.split(" ").join(""))+".mp3");
...
bugBuild.prototype.combat = function(){
 ...
 if(this.Animation < 3&&bugStats[this.Species].skillName.split("Ambush").length > 1){       
  ...
  if(this.Swimming){ this.bubbleCreate(bugStats[this.Species].swimAble ? 15 : 10); }
  if(this.Albino&&this.X-scrollx > -10&&this.X < 540+scrollx){ this.bubbleCreate(3,1); }
...
bugBuild.prototype.keyDown = function(event){
 ...
 if(boxSelector == ""&&(event.keyCode == 39||event.keyCode == 68||event.keyCode == 37||event.keyCode == 65||event.keyCode == 38||event.keyCode == 87||event.keyCode == 32||event.keyCode == 40||event.keyCode == 83)){ this.keyUp[event.keyCode] = true; }

 if(boxSelector == ""&&(event.keyCode == 39||event.keyCode == 68)&&!this.Attacking&&!this.defects.speedMax){ this.Scale = 1; }
 if(boxSelector == ""&&(event.keyCode == 37||event.keyCode == 65)&&!this.Attacking&&!this.defects.speedMax){ this.Scale = -1; }
 ...
 if(boxSelector == ""&&event.keyCode >= 49&&event.keyCode < 53&&event.keyCode-48 <= battleBugs.length&&!this.Attacking&&this.Health > 0) { 
    ...
    if(bugSelected == this) { bugSelected = battleBugs[event.keyCode-47]; }
...
bugBuild.prototype.draw = function(){
  ...
  if(!this.Enemy&&!battleMode){ 
    ...
    ctx.save();
    ctx.globalAlpha = 1;
    if(battleMode&&bugStats[this.Species].skillName == "Specialized Ambush"){ ctx.globalAlpha = 1-this.Fury }
    ...
    if(boxSelector == ""&&this.Health <= 0&&this.Rotate > -180&&textInfo.length == 1&&(battleMode||!this.Immortal)){ 
       ...
       if(bugSelected == this){ bugSelected = -1; }
       ...
       if(!this.Enemy) {  
             ...
             if(!this.Immortal){ bugs.splice(bugs.indexOf(this),1); }  
   ...
   if((this.Alignments > 0&&this.Alignments < 360)||this.Albino||this.defects.evolution){
    ...
    if(textInfo.length > 1||boxSelector == "End Game"){ return }
    ...
    this.Speed = 8;
    if(this.defects.droneType == undefined){ this.Fury = Math.max(0.5,this.Fury) }
    } else {
    this.defects.speedMax = false;
    }
    
    // Health degradation for drones
    if(boxSelector == ""&&this.defects.droneType !== undefined){ this.Health = Math.max(0,this.Health-this.HealthTotal*1/60) }
    ...
    if(this.defects.poison % 1 < 1/30&&this.defects.poison > 0){ this.defects.hurt = [2,1]; this.Health -= 2; }
    }
    
    if(this.Swimming&&!bugStats[this.Species].swimAble){ this.Speed /= 2 }
    ...
    if(this.Fury > 1){ this.Fury = 1 }
    if(this.Fury > 0){ this.Fury = Math.max(0,this.Fury-1/120) }  
    ...
    ctx.fillRect(this.X-scrollx-this.Image.width/72*(this.Age/150+0.5)/3,this.Y-this.Image.height/19.5*(this.Age/150+0.5),(this.Image.width/72*(this.Age/150+0.5)/1.5)/this.HealthTotal*this.Health,this.Image.height/45*(this.Age/150+0.5)/4);
        ...
        if(battleMode){
            ...
            if(this.Health <= this.HealthTotal*0.25&&this.Trait == 1){ this.Fury = 1; }  
            ...
            this.AlignmentsText = 
             [(100/90)*(90-Math.abs(90-this.Alignments)) > 0 ? (100/90)*(90-Math.abs(90-this.Alignments)) : 0,
    ...
    if(boxSelector == ""&&this.defects.evolution > 0){
     if(this.defects.evolution == 1){ soundeffect("Hatch.mp3"); }
    ...
    if(battleBugs.length == 0&&this.Enemy){ this.defects = {} };
 ...
 if(boxSelector == ""&&this.Health > 0&&(this.keyUp[39]||this.keyUp[68])&&!(this.keyUp[37]||this.keyUp[65])){ this.Scale = 1 } else if(boxSelector == ""&&this.Health > 0&&!(this.keyUp[39]||this.keyUp[68])&&(this.keyUp[37]||this.keyUp[65])) { this.Scale = -1 }
 ...
 if(this.Swimming&&this.X-scrollx > -10&&this.X < 540+scrollx){ this.bubbleCreate(bugStats[this.Species].swimAble ? 10 : 5); }
 if(this.Albino&&this.X-scrollx > -10&&this.X < 540+scrollx){ this.bubbleCreate(3,1); }
...
this.Animation = 0.1;
 ...
 if(boxSelector == ""&&!this.Enemy&&this.Health > 0&&this.Hold&&!this.Attacking&&this.Trait == 0){ 
     ...
     if(this.Health % 1 <= 1/30&&this.Health < this.HealthTotal){ this.defects.hurt = [-1,1]; }
 } 
 ...
 //constraints
 if(this.Y < 40&&boxSelector == ""){ this.Y = 40; this.Jump = 5; }   
 if(this.Y > 310&&boxSelector == ""){ this.Enemy && bugStats[this.Species].flyAble ? this.Jump = 10 : this.Health = 0; }    
 ...
 if(this.Health > 0&&(this.keyUp[38]||this.keyUp[87])&&((this.Jump == 0&&this.Hold == true)||bugStats[this.Species].flyAble)&&!this.Attacking&&boxSelector == ""){ 
     if(bugStats[this.Species].flyAble&&this.Jump == 0&&this.Hold == true){ soundeffect("FlyStart.mp3"); }
     this.Jump = 10 
     } 
 if((this.keyUp[40]||this.keyUp[83])&&this.Jump == 10&&bugStats[this.Species].flyAble&&!this.Attacking&&boxSelector == ""){ this.Jump = 5 }   
...
//Non enemies can jump with up arrow keys or shadow hightail/player drone, they can attack if they have range with Venom Spray or Ambush
 ...
 if(this.Y > 278&&!battleMode){ this.Y = 278; }
 this.Hold = false;
 this.Swimming = false; 
}
