...
battleBugs.sort(function(a,b){ return bugs.indexOf(a) == -1 ? b : bugs.indexOf(a)-bugs.indexOf(b) });
...
battleLoot = function(mission){
    let loot = [["Territs",1,1,1],[1,1,1,1]];
...
battleClose = function(){
    ...
    if(missionCalculate < 14){ 
        ...
        while(missionCalculate == battleInfo[0]) {  missionCalculate = 15+Math.floor(Math.random()*5); }
        rooms[0].MissionList[rooms[0].MissionSelect] = [missionCalculate,battleLoot(missionCalculate)]; 
    } else if(missionCalculate < 25)  {
        while(missionCalculate == battleInfo[0]) {  missionCalculate = 20+Math.floor(Math.random()*5); }
        rooms[0].MissionList[rooms[0].MissionSelect] = [missionCalculate,battleLoot(missionCalculate)]; 
    } else if(missionCalculate < 30)  {
        while(missionCalculate == battleInfo[0]) {  missionCalculate = 25+Math.floor(Math.random()*5); }
        rooms[0].MissionList[rooms[0].MissionSelect] = [missionCalculate,battleLoot(missionCalculate)]; 
    }
...
buttonmaker = function(text,x,y,size,action){
    ...
    if(mousedown&&ctx.globalAlpha == 1){ action() }
...
weatherDraw = function(){ 
   if(weather[1] <= 5){ weather[1] = Math.min(5,weather[1]+1/30)
   } else { 
 ...
 if(weather[0] == "Snowstorm"){ weather[weatherEffect][0] += (weather[weatherEffect][1] % 3)-1 ; }
         
 ctx.restore();
    
  if(weather[weatherEffect][1] > Math.random()*15+260){ 
   weather[weatherEffect][0] = Math.random()*528+scrollx;                                                                                     weather[weatherEffect][1] = -50-Math.random()*250;
    if(weather[0] !== "Snowstorm"){ soundeffect("Rain.mp3"); }
  }
    ...
    //Every 12 seconds there is thunder in a thunderstorm
       if(weather[0] == "Thunderstorm"&&date.getSeconds() % 12 == 0&weather[1] <= 5){
         ...
         if(date.getMilliseconds() < 50){ soundeffect("Thunder.mp3"); }
     }   
     ...
     if(ctx.globalAlpha == 1){ 
         ...
         if(mousedown&&boxSelector == "Bug Selector Full"){ 
             ...
             if(itemAction == "Randomize") {
                 ...
                 while(bugSelected.Trait == uniqueTrait){ uniqueTrait = Math.floor(Math.random()*traitDescp.length); }
                 ...
                 bugSelected.AlignmentsText = 
             [(100/90)*(90-Math.abs(90-bugSelected.Alignments)) > 0 ? (100/90)*(90-Math.abs(90-bugSelected.Alignments)) : 0,
...
ctx.globalAlpha = collision(mousex,mousey,0,0,x,y,30,30) ? 1 : 0.85;
    ...
    if(ctx.globalAlpha == 1){
        ...
        if(mousedown&&boxSelector == "Bug Selector History"&&bugSelected.Age >= 100){ boxSelector = "Bug Selector Item"; bugSelected = -1; }
    }
...
mousemake = function(event) {
    ...
    if(textInfo.length == 1&&boxSelector == ""){ music.play(); }
}
...
keydownmisc = function(event){
  if(boxSelector.split("Select Facility ").length > 1){
    ...
    if((event.keyCode == 39||event.keyCode == 68)){ boxSelector = (facilityIndex >= rooms.length-1 ? "Select Facility 0" : "Select Facility "+(facilityIndex+1)) }
    if((event.keyCode == 37||event.keyCode == 65)){ boxSelector = (facilityIndex-1 < 0 ? "Select Facility "+(rooms.length-1) : "Select Facility "+(Number(boxSelector.split(" Facility ")[1])-1)) }
      
  } else if(boxSelector == "Guide"){
    if((event.keyCode == 39||event.keyCode == 68)){ page + 1 > 4 ? page = 0 : page += 1 }
    if((event.keyCode == 37||event.keyCode == 65)){ page - 1 < 0 ? page = 4 : page -= 1 }
      
  } else if(boxSelector == "Bug Compendium"){
    if((event.keyCode == 39||event.keyCode == 68)){ page + 1 > bugCompendium.length-1 ? page = 0 : page += 1 }
    if((event.keyCode == 37||event.keyCode == 65)){ page - 1 < 0 ? page = bugCompendium.length-1 : page -= 1 }
      
  } else if(!filterSelected&&boxSelector.substr(0,12) == "Bug Selector"){
    if((event.keyCode == 39||event.keyCode == 68)){ page + 1 > Math.floor((nameFilter.length-1)/8) ? page = 0 : page += 1 } 
    if((event.keyCode == 37||event.keyCode == 65)){ page - 1 < 0 ? page = Math.floor((nameFilter.length-1)/8) : page -= 1 }
...
bugTotal = function(){
    ...
    for(totalAdd in bugCompendium) {
     if (bugStats[bugCompendium[totalAdd]].obtained) { total += 1 }
    }
    return total
}
...
shopHandle = function(x,y,index,thumbnail){  
 ...
 ? "brightness(100%)" : "brightness(80%)"
     ...
     if(date.getDay() == 0 || date.getDay() == 6){ textmaker("2x value!",x+190,y+100,15,true); } 
 }
}
...
load = function(deleteFile = false){
    ...
    bugs[bugImage].defects = {};
       if(bugs[bugImage].Patrol.Type == 0){ battleBugs.push(bugs[bugImage]); } 
    }
    ...
    for(let refresh = 0;refresh < 10;refresh++){ items[refresh].quantity = 0; }
    for(bugIndex in bugCompendium){ bugStats[bugCompendium[bugIndex]].obtained = false; }
    territs = 25;
    food = 500;
